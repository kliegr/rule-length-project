...
np.random.seed(0)
df=pd.read_csv('output/mercer.arules.pairs.filtered', delimiter=",", quoting=csv.QUOTE_ALL)
dfInvertedHeuristics=pd.read_csv('output/mercer.invertedheuristics.pairs.filtered', delimiter=",", quoting=csv.QUOTE_ALL)
#dfInvertedHeuristics=dfInvertedHeuristics.drop_duplicates(subset=["r1","r2"])
...
out.to_csv("crowdflower_input/mercer.raw.csv",columns=columns,quoting=csv.QUOTE_NONNUMERIC,index_col="id")
