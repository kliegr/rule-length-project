...
def generateText(column):
 return column.replace([" => {Label=","}$","{","}",",","=1.0"],["then the quality of living is <b>","</b>" ,"if the city falls into all of the following group(s) (simultaneously) <ul><li>", "&nbsp;</li></ul>", "&nbsp;<b>and</b> </li><li>",""], regex=True)
...
def generateClues(correct,coverage,label,confidence):
 ...
 if float(confidence)==1.0:
  return "Following cities fall into all of the groups defined by the rule: <b>" +niceListOfInstances(correct)+ "</b><br/> All these countries have a risk of traffic accidents <b>" + label +"</b>"
 else:
  return "Following cities fall into all of the groups defined by the rule: <b>" +niceListOfInstances(coverage) + " </b><br/>For "  + str(correct.count(",")+1) + " of these cities ("+niceListOfInstances(correct) +"), the rule correctly predicts the quality of living as <b>" + label + "</b>." 
 
 
dfData=pd.read_csv('crowdflower_input/mercer.raw.csv', delimiter=",", quoting=csv.QUOTE_ALL)
...
dfData[dfData.tag!="TEST"].to_csv("crowdflower_input/mercer.csv",columns=columns,quoting=csv.QUOTE_NONNUMERIC,index_label="id")
#needs to be manually edited afterwards
dfData[dfData.tag=="TEST"].to_csv("testquestions/mercer.test",columns=columns,quoting=csv.QUOTE_NONNUMERIC,index_label="id")


dfMap.to_csv('crowdflower_input_literal_importance/mercer.name.map', delimiter=",", quoting=csv.QUOTE_ALL,index_label="id")
